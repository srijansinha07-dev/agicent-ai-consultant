...
def _website_concept_prompt(query: str, context: str) -> str:
    from services.website_support import WEBSITE_CONTEXT_CHAR_LIMIT, WEBSITE_SUPPORT_RULES

    ctx = context[:WEBSITE_CONTEXT_CHAR_LIMIT]
    return (
        f"{WEBSITE_SUPPORT_RULES}\n\n"
        f"CONTEXT:\n{ctx}\n\n"
        f"QUESTION: {query}\n\n"
        "Synthesize across relevant context. Choose a structure that fits the user's intent "
        "(short bullets, brief sections, or a compact paragraph). "
        "Stay within 150–250 words. Speak as Agicent. No chunk labels or retrieval meta-commentary."
    )
...
def _build_prompt(query, context, qtype, page_num, ocr_used, doc_id: str = "") -> str:
    ...
    if doc_id:
        try:
            ...
            if is_website_doc(doc_id) and qtype == QueryType.CONCEPT:
                return _website_concept_prompt(query, context)
        except Exception:
            pass
...
def _website_conversational_reply(query: str) -> str | None:
    q = query.strip().lower()
    if not q:
        return None

    greeting_re = re.compile(r"^(hi|hello|hey|good morning|good afternoon|good evening|yo|hiya)[!. ]*$")
    ack_re = re.compile(
        r"^(thanks|thank you|ok|okay|got it|understood|sounds good|cool|great|nice|perfect|makes sense)[!. ]*$"
    )

    if greeting_re.match(q):
        return (
            "Hi! Happy to help. If you're planning a product, AI feature, team setup, or budget, "
            "share your goal and constraints and I can suggest a practical next step."
        )
    if ack_re.match(q):
        return "You got it. If you want, I can help you compare options or outline the next concrete step."
    return None
...
def node_generate_answer(state: GraphState) -> GraphState:
    ...
    try:
        ...
        try:
            ...
            if is_website_doc(state["doc_id"]):
                max_tokens = 380
        except Exception:
            pass
        print("\n===== CONTEXT DEBUG =====")
...
def run_chat_graph(
    ...
    # Keep small-talk conversational in website consultant mode instead of forcing long consulting responses.
    try:
        from services.website_support import is_website_doc
        from models import QueryType

        if is_website_doc(doc_id):
            conversational = _website_conversational_reply(query)
            if conversational:
                return {
