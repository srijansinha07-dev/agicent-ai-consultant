...
def retrieve(
    ...
    final_k    = None
    try:
        from services.website_support import is_website_doc, WEBSITE_TOP_K_FINAL

        if is_website_doc(doc_id):
            final_k = WEBSITE_TOP_K_FINAL
    except Exception:
        pass
    final = _rerank(query, candidates, top_k=final_k)
...
def _hybrid(
    ...
    website = False
    apply_website_metadata_boosts = None
    sem_k_default = max(TOP_K_SEMANTIC, 10)
    bm25_k_default = 10
    candidate_cap = 8
    try:
        from services.website_support import (
            WEBSITE_CANDIDATE_CAP,
            WEBSITE_TOP_K_BM25,
            WEBSITE_TOP_K_SEMANTIC,
            apply_website_metadata_boosts as _apply_website_boosts,
            expand_website_retrieval_query,
            is_website_company_query,
            is_website_doc,
        )

        apply_website_metadata_boosts = _apply_website_boosts
        website = is_website_doc(doc_id)
        if website:
            sem_k_default = WEBSITE_TOP_K_SEMANTIC
            bm25_k_default = WEBSITE_TOP_K_BM25
            candidate_cap = WEBSITE_CANDIDATE_CAP
        if website and is_website_company_query(query):
            query = expand_website_retrieval_query(query)
            query_lower = query.lower()
    except Exception:
        pass
    ...
    # ── General / concept queries ──────────────────────
        ...
        for c in candidates:
            ...
            if website and apply_website_metadata_boosts:
                apply_website_metadata_boosts(c)
...
def _rerank(query: str, chunks: list[Chunk], top_k: int | None = None) -> list[Chunk]:
    if not chunks:
        return []
    k = top_k if top_k is not None else TOP_K_FINAL
