    ...
    return AgentState(
        **{
            **state.__dict__,
            ...
            "context": _build_context(result.chunks, state.doc_id),
        }
    )
...
def _ground_answer(state: AgentState, task_prompt: str) -> AgentState:
    ...
    try:
        from services.website_support import WEBSITE_SUPPORT_RULES, is_website_doc

        if is_website_doc(state.doc_id):
            base_rules = f"{WEBSITE_SUPPORT_RULES}\n\n"
    except Exception:
        pass
...
def _run_summary_synthesis_workflow(state: AgentState, mode: str, query_hint: str) -> dict:
    ...
    merged_context = _build_context(merged_chunks, state.doc_id)
...
def _build_context(chunks: list, doc_id: str = "") -> str:
    limit = CONTEXT_CHAR_LIMIT
    if doc_id:
        try:
            from services.website_support import WEBSITE_CONTEXT_CHAR_LIMIT, is_website_doc

            if is_website_doc(doc_id):
                limit = WEBSITE_CONTEXT_CHAR_LIMIT
        except Exception:
            pass
